function rp_mk_dataimage, mpars, bkg, window, psf, $
                          nopsf=nopsf, $
...
if not keyword_set(parent_scale) then parent_scale=3d
if not keyword_set(centroid_tol) then centroid_tol=1.5d
if not keyword_set(max_iter) then max_iter=10

; This function uses RP_MK_IMAGE to create a data image.  The main
...
parent_image=rp_mk_image(mpars,bkg,parent_window,psf,$
